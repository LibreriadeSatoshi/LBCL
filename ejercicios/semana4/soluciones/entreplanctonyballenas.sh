...
semana4 () {
	...
	banner semana4
	echo -e "$NC********************************"
	echo -e "$YELL Configurar un contrato Timelock $NC"
	echo "********************************"
	echo "1. Crea tres monederos: Miner, Empleado y Empleador."
	echo "********************************"
		bitcoin-cli -named createwallet wallet_name="Miner" descriptors=true | jq
		bitcoin-cli -named createwallet wallet_name="Empleado" descriptors=true | jq
		bitcoin-cli -named createwallet wallet_name="Empleador" descriptors=true | jq
	echo "********************************"
	echo "2. Fondea los monederos generando algunos bloques para Miner y enviando algunas monedas al Empleador."
	echo "********************************"
		reward=`bitcoin-cli -rpcwallet=Miner getnewaddress "Recompensa"`
		echo -e "Nueva direccion en la cartera Miner: $YELL $reward $NC"
		bitcoin-cli generatetoaddress 103 "$reward"|jq
		saldominer=`bitcoin-cli -rpcwallet=Miner getbalance`
		echo -e "Saldo inicial en la cartera Miner: $YELL $saldominer $NC"
		# nuevas direcciones en Empleado y Empleador
		addrfondeoEmpleador=`bitcoin-cli -rpcwallet=Empleador getnewaddress "Empleador monedas"`
		echo -e "Nueva direccion en la cartera $MAG Empleador: $addrfondeoEmpleador $NC"
		txEmpleador=`bitcoin-cli -rpcwallet=Miner sendtoaddress "$addrfondeoEmpleador" 100`
		echo -e "Id de transaccion hacia $MAG Empleador: $txEmpleador $NC"
		echo -e "$YELL Confirmando transacciones en nuevo bloque $NC" 
		bitcoin-cli generatetoaddress 1 "$reward" | jq
		saldoEmpleador=`bitcoin-cli -rpcwallet=Empleador getbalance`
		echo -e "Balance actual en wallet $MAG Empleador $saldoEmpleador $NC"
		sleep 3
	echo "********************************"
	echo "3. Crea una transacción de salario de 40 BTC, donde el Empleador paga al Empleado."
	echo "********************************"			
		addrfondeoEmpleado=`bitcoin-cli -rpcwallet=Empleado getnewaddress "Empleado monedas"`
		echo -e "Nueva direccion en la cartera $BLU Empleado: $addrfondeoEmpleado $NC"
		# txEmpleado=`bitcoin-cli -rpcwallet=Miner sendtoaddress "$addrfondeoEmpleado" 50`
		addrCambioEmpleador=`bitcoin-cli -rpcwallet=Empleador getnewaddress "Empleador Cambio"`
		sleep 3
	echo "********************************"
	echo "4. Agrega un timelock absoluto de 500 bloques para la transacción, es decir, la transacción no puede incluirse en el bloque hasta que se haya minado el bloque 500."
	echo "********************************"
		alturaactual=$(bitcoin-cli getblockcount|bc)
		echo "Altura actual de bloques: $alturaactual"
		dentrode500=$(echo "$alturaactual + 500"|bc)
		echo "Altura dentro de 500 bloques: $dentrode500"
		txEmpleado=$(bitcoin-cli -named createrawtransaction inputs='''[ { "txid": "'$txEmpleador'", "vout": '1' } ]''' outputs='''{ "'$addrfondeoEmpleado'": 40, "'$addrCambioEmpleador'": 59.999998 }''' locktime=$dentrode500)		
		echo -e "Hex de transaccion hacia $BLU Empleado: $txEmpleado $NC"
		bitcoin-cli decoderawtransaction $txEmpleado |jq 
		sleep 3
	echo "********************************"
	echo "5. Informa en un comentario qué sucede cuando intentas transmitir esta transacción."
	echo "********************************"
		signedtxEmpleado=$(bitcoin-cli -rpcwallet=Empleador -named signrawtransactionwithwallet hexstring=$txEmpleado | jq -r '.hex')
		echo -e "HEX de la transaccion firmada: $MAG $signedtxEmpleado $NC"
		#txidEmpleado=$(bitcoin-cli -named sendrawtransaction hexstring=$signedtxEmpleado)
		echo -e "Despues de firmar la transaccion, se intenta enviar y tenemos el siguiente mensaje: $YELL $txidEmpleado"
		bitcoin-cli -named sendrawtransaction hexstring=$signedtxEmpleado 
		echo -e "$MAG Este error ocurre porque nuestra cadena de bloques aun no ha llegado a la altura deseada para poder transmitir la transacción $NC"
	echo "********************************"
	echo "6. Mina hasta el bloque 500 y transmite la transacción."
	echo "********************************"
		bitcoin-cli generatetoaddress 500 "$reward" | jq
		alturaactual=$(bitcoin-cli getblockcount|bc)
		txidEmpleado=$(bitcoin-cli -named sendrawtransaction hexstring=$signedtxEmpleado)
		echo -e "En la altura de bloque $BLU $alturaactual $NC, se intenta enviar  la transaccion y tenemos el siguiente mensaje: $YELL $txidEmpleado $NC"
	echo "********************************"
	echo "7. Imprime los saldos finales del Empleado y Empleador."
	echo "********************************"
		bitcoin-cli generatetoaddress 1 "$reward" | jq
		saldoEmpleador=`bitcoin-cli -rpcwallet=Empleador getbalance`
		echo -e "Balance actual en wallet $MAG Empleado $saldoEmpleador $NC"
		saldoEmpleado=`bitcoin-cli -rpcwallet=Empleado getbalance`
		echo -e "Balance actual en wallet $BLU Empleado $saldoEmpleado $NC"
		sleep 3
	echo "********************************"		
	echo -e "$YELL Gastar desde el Timelock $NC"
	echo "********************************"		
	echo "1. Crea una transacción de gasto en la que el Empleado gaste los fondos a una nueva dirección de monedero del Empleado."
	echo "********************************"			
		addrsalarioEmpleado=`bitcoin-cli -rpcwallet=Empleado getnewaddress "Empleado salario"`
	echo -e "Nueva direccion en la cartera $BLU Empleado: $addrsalarioEmpleado $NC"
	echo "********************************"		
	echo "2. Agrega una salida OP_RETURN en la transacción de gasto con los datos de cadena "He recibido mi salario, ahora soy rico"."
	echo "********************************"		
		...
		txsalarioEmpleado=$(bitcoin-cli -named createrawtransaction inputs='''[ { "txid": "'$txidEmpleado'", "vout": '0' } ]''' outputs='''{ "data": "'$op_return_datos'", "'$addrsalarioEmpleado'": 39.99999800 }''')		
		echo -e "Hex de transaccion saliario de $BLU Empleado: $txsalarioEmpleado $NC"
		bitcoin-cli decoderawtransaction $txsalarioEmpleado |jq 
		sleep 3
	echo "********************************"			
	echo "3. Extrae y transmite la transacción completamente firmada."
	echo "********************************"			
		signedtxsalarioEmpleado=$(bitcoin-cli -rpcwallet=Empleado -named signrawtransactionwithwallet hexstring=$txsalarioEmpleado | jq -r '.hex')
		txidsalarioEmpleado=$(bitcoin-cli -named sendrawtransaction hexstring=$signedtxsalarioEmpleado)
		echo -e "Id de transaccion salario de $BLU Empleado: $txidsalarioEmpleado $NC"
	echo "********************************"			
	echo "4. Imprime los saldos finales del Empleado y Empleador."
	echo "********************************"			
		echo -e "$YELL Confirmando transacciones en nuevo bloque $NC" 
		bitcoin-cli generatetoaddress 1 "$reward" | jq
		echo -e "Mensaje en hexadecimal en el $YELL OP_RETURN $NC del script de bloqueo:" 
		bitcoin-cli getrawtransaction $txidsalarioEmpleado 1 |jq '.vout[0]| .scriptPubKey|.asm'
		saldoEmpleado=`bitcoin-cli -rpcwallet=Empleado getbalance`
		echo -e "Balance actual en wallet $BLU Empleado $saldoEmpleado $NC"
		saldoEmpleador=`bitcoin-cli -rpcwallet=Empleador getbalance`
		echo -e "Balance actual en wallet $MAG Empleador $saldoEmpleador $NC"
