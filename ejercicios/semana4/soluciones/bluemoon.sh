...
#Semana 4
transaccion_timelock(){
	#Crea tres monederos: Miner, Empleado y Empleador.
	echo "**************************************"
	echo -e "\n${redColour}[+]${endColour}${blueColour}Crea tres monederos: Miner, Empleado y Empleador.${endColour}\n"
	
		bitcoin-cli -regtest -named createwallet wallet_name=Miner > /dev/null
		bitcoin-cli -regtest -named createwallet wallet_name=Empleado > /dev/null
		bitcoin-cli -regtest -named createwallet wallet_name=Empleador > /dev/null
	
	#Fondea los monederos generando algunos bloques para Miner y enviando algunas monedas al Empleador.
	echo "**************************************"
	echo -e "\n${redColour}[+]${endColour}${blueColour}Fondea los monederos generando algunos bloques para Miner y enviando algunas monedas al Empleador.${endColour}\n"
	...
	echo 
	#Crear una dirección receptora para el Empleador.
	echo "**************************************"
	echo -e "\n${redColour}[+]${endColour}${blueColour}Crear una dirección receptora para el Empleador.${endColour}\n"
		echo 
		echo "*************************************"
		echo "*************Empleador****************"
		echo "*************************************"
		
		address_empleador=$(bitcoin-cli -rpcwallet=Empleador getnewaddress "Empleador Recibido")
		echo -e "\n${redColour}[+]${endColour}${blueColour}Direccion de Empleador:${endColour} ${yellowColour} ${address_empleador}.${endColour}\n"
	
	#Enviar 50 BTC desde la billetera Miner a la billetera del Empleador.
		echo "**************************************"
		echo -e "\n${redColour}[+]${endColour}${blueColour}Enviar 50 BTC desde la billetera Miner a la billetera del Empleador.${endColour}\n"

		#Enviar sats
		txid_empleador=$(bitcoin-cli -rpcwallet=Miner sendtoaddress ${address_empleador} 70 "Envio a Empleador")

		echo "**************************************"
		echo -e "\n${redColour}[+]${endColour}${blueColour}Transaccion de Empleador: ${endColour} ${yellowColour}${txid_empleador}.${endColour}\n"
	...
	#Imprimir el saldo del Empleador
	echo "**************************************"
	echo -e "\n${redColour}[+]${endColour}${blueColour}Saldo de la billetera del Empleador.${endColour}\n"
		bitcoin-cli -rpcwallet=Empleador getbalance 
		
	#Crea una transacción de salario de 40 BTC, donde el Empleador paga al Empleado.
	echo "**************************************"
	echo -e "\n${redColour}[+]${endColour}${blueColour}Crea una transacción de salario de 40 BTC, donde el Empleador paga al Empleado.${endColour}\n"
	echo
	
	#Crear una dirección receptora para el Empleado.
	echo "**************************************"
	echo -e "\n${redColour}[+]${endColour}${blueColour}Crear una dirección receptora para el Empleado.${endColour}\n"
		echo 
		echo "*************************************"
		echo "*************Empleado****************"
		echo "*************************************"
		
		address_empleado=$(bitcoin-cli -rpcwallet=Empleado getnewaddress "Empleado Recibido")
		echo -e "\n${redColour}[+]${endColour}${blueColour}Direccion de Empleado:${endColour} ${yellowColour} ${address_empleado}.${endColour}\n"
	
	echo -e "\n${redColour}[+]${endColour}${blueColour}Crear una dirección de cambio para el Empleador.${endColour}\n"
		echo 
		echo "*************************************"
		echo "*************Empleador****************"
		echo "*************************************"	
		
		address_cambio_empleador=$(bitcoin-cli -rpcwallet=Empleador getnewaddress "Cambio Empleador")
		echo -e "\n${redColour}[+]${endColour}${blueColour}Direccion de cambio para el Empleador:${endColour} ${yellowColour} ${address_cambio_empleador}.${endColour}\n"
	...
	echo "**************************************"
	echo -e "\n${redColour}[+]${endColour}${blueColour}Agrega un timelock absoluto de 500 bloques para la transacción, es decir, la transacción no puede incluirse en el bloque hasta que se haya minado el bloque 500.${endColour}\n"
		getblock=$(bitcoin-cli getblockcount | bc)
		timelock500=$(echo "$getblock + 500" | bc)
		
	echo "**************************************"
	echo -e "\n${redColour}[+]${endColour}${blueColour}Estamos en el bloque: ${endColour} ${yellowColour} ${getblock}.${endColour}\n"
	echo 
	echo "**************************************"
	echo -e "\n${redColour}[+]${endColour}${blueColour}Crear la transacción${endColour}\n"
		tx_timelock=$(bitcoin-cli -named createrawtransaction inputs='''[ { "txid": "'$txid_empleador'", "vout": '0' } ]''' outputs='''{ "'$address_empleado'": 40, "'$address_cambio_empleador'": 29.999998 }''' locktime=$timelock500)	
	
	#Informa en un comentario qué sucede cuando intentas transmitir esta transacción.
	echo "**************************************"
	echo -e "\n${redColour}[+]${endColour}${blueColour}Informa en un comentario qué sucede cuando intentas transmitir esta transacción.${endColour}\n"
	
		sig_timelock500=$(bitcoin-cli -rpcwallet=Empleador -named signrawtransactionwithwallet hexstring=$tx_timelock | jq -r '.hex')	
		bitcoin-cli -named sendrawtransaction hexstring=$sig_timelock500
	
	sleep 3s
		echo "*************************************"
		echo -e "\n${redColour}[+]${endColour}${blueColour}Este error es porque todavìa no llegamos a es bloque estamos en el bloque:${endColour}${yellowColour}${getblock}.${endColour}\n"
		echo "*************************************"	
	
	#Mina hasta el bloque 500 y transmite la transacción.
	echo -e "\n${redColour}[+]${endColour}${blueColour}Mina hasta el bloque 500 y transmite la transacción.${endColour}\n"
		bitcoin-cli generatetoaddress 500 $address_miner >/dev/null
		getblock=$(bitcoin-cli getblockcount | bc)
	echo
	echo "**************************************"
	echo -e "\n${redColour}[+]${endColour}${blueColour}Estamos en el bloque: ${endColour}${yellowColour}${getblock}${endColour} se puede transmitir.${endColour}\n"		
		send_timelock500=$(bitcoin-cli -named sendrawtransaction hexstring=$sig_timelock500)
	
	#Imprime los saldos finales del Empleado y Empleador.
		bitcoin-cli generatetoaddress 1 $address_miner >/dev/null
	echo "**************************************"
	echo -e "\n${redColour}[+]${endColour}${blueColour}Imprime los saldos finales del Empleado y Empleador.${endColour}\n"
	echo "**************************************"
	echo -e "\n${redColour}[+]${endColour}${blueColour}Saldo empleador:${endColour}\n"
		bitcoin-cli -rpcwallet=Empleador getbalance 
	echo "**************************************"
	echo -e "\n${redColour}[+]${endColour}${blueColour}Saldo empleado:${endColour}\n"
	...
	echo "**************************************"
	echo -e "\n${redColour}[+]${endColour}${blueColour}Crea una transacción de gasto en la que el Empleado gaste los fondos a una nueva dirección de monedero del Empleado.${endColour}\n"
	
		address_gasto_empleador=$(bitcoin-cli -rpcwallet=Empleado getnewaddress "Empleado gasto")
		echo -e "\n${redColour}[+]${endColour}${blueColour}Direccion de gasto del empleado:${endColour} ${yellowColour} ${address_gasto_empleador}.${endColour}\n"
		
	#Agrega una salida OP_RETURN en la transacción de gasto con los datos de cadena "He recibido mi salario, ahora soy rico".
	echo "**************************************"
	echo -e "\n${redColour}[+]${endColour}${blueColour}Agrega una salida OP_RETURN en la transacción de gasto con los datos de cadena He recibido mi salario, ahora soy rico.${endColour}\n"
		str="He recibido mi salario, ahora soy rico"
		hex=$(printf '%s' "$str" | xxd -p -u)
		hex_without_space=$(echo "$hex" | tr -d '[[:space:]]')
		
	echo "**************************************"
	echo -e "\n${redColour}[+]${endColour}${blueColour}Crear transacción.${endColour}\n"	
		tx_op_return=$(bitcoin-cli -named createrawtransaction inputs='''[ { "txid": "'$send_timelock500'", "vout": '0' } ]''' outputs='''{ "data": "'$hex_without_space'", "'$address_gasto_empleador'": 39.99999800 }''')	
		bitcoin-cli decoderawtransaction $tx_op_return | jq 
	
	sleep 3s
	#Extrae y transmite la transacción completamente firmada.
	echo "**************************************"
	echo -e "\n${redColour}[+]${endColour}${blueColour}Extrae y transmite la transacción completamente firmada.${endColour}\n"	
		sig_op_return=$(bitcoin-cli -rpcwallet=Empleado -named signrawtransactionwithwallet hexstring=$tx_op_return | jq -r '.hex')	
		send_op_return=$(bitcoin-cli -named sendrawtransaction hexstring=$sig_op_return)
