...
week_4() {
    ...
    TEXT="Salario xWaZzo 40 BTC bien ganados!"
    info_flag "Vamos a escribir en OP_RETURN: $TEXT"
    OP_RETURN_DATA=$(printf "%s" "$TEXT" | xxd -p | tr -d '\n')
    info_flag "Text in HEX $OP_RETURN_DATA"
    ...
    RAW_TRX=$(bitcoin-cli -named createrawtransaction inputs="[$EMPLOYER_BLOCK]" outputs="[{\"data\": \"$OP_RETURN_DATA\"},{\"$EMPLOYEE_ADDRESS\": $EMPLOYEE_SALARY},{\"$EMPLOYER_ADDRESS\": $EMPLOYER_CHANGE}]" locktime="$LOCKTIME")
